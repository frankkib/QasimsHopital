  ...
  return (
    <nav className="p-4 flex flex-col md:flex-row justify-between items-center">
      <div className="flex items-center justify-between w-full md:w-auto">
        <img src={reactLogo} alt="React Logo" className="h-16 w-16 rounded-full" />
        <button className="md:hidden" onClick={toggleMenu}>
          {isOpen ? <FaTimes className="h-6 w-6" /> : <FaBars className="h-6 w-6" />}
        </button>
      </div>
      <ul className={`flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-6 ${isOpen ? 'block' : 'hidden'} md:flex w-full md:w-auto`}>
        <li className="flex items-center space-x-2">
          <FaHome className="hover:text-green-300 h-6 w-6 transition-transform duration-300 hover:scale-110" />
          <Link to="/" className="hover:text-green-300 transition-colors duration-300 font-sans">Home</Link>
        </li>
        <li className="relative flex items-center space-x-2 dropdown">
          <FaInfoCircle className="hover:text-green-300 h-6 w-6 transition-transform duration-300 hover:scale-110" />
          <button onClick={toggleAboutDropdown} className="hover:text-green-300 flex items-center transition-colors duration-300 font-sans">
        ...
        <li className="relative flex items-center space-x-2 dropdown">
          <FaServicestack className="hover:text-green-300 h-6 w-6 transition-transform duration-300 hover:scale-110" />
          <button onClick={toggleServicesDropdown} className="hover:text-green-300 flex items-center transition-colors duration-300 font-sans">
            Services <FaCaretDown className="ml-1" />
          </button>
          {isServicesDropdownOpen && (
            <ul className="absolute top-full left-0 mt-2 w-64 bg-white text-black rounded-lg shadow-lg">
              <li className="px-4 py-2 hover:bg-green-100 transition-colors duration-300 rounded-lg">
              ...
              <li className="px-4 py-2 hover:bg-green-100 transition-colors duration-300 rounded-lg">
                <Link to="/services" className="block">GBV & HIV Management & Support</Link>
              </li>
            </ul>
          )}
        </li>
        <li className="flex items-center space-x-2">
          <FaEnvelope className="hover:text-green-300 h-6 w-6 transition-transform duration-300 hover:scale-110" />
          <Link to="/contact" className="hover:text-green-300 transition-colors duration-300 font-sans">Contact</Link>
        </li>
      </ul>
    </nav>
  );
